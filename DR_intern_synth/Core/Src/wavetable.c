...
void wavetable_create_sine(
		uint16_t* out, uint16_t ref_v, uint16_t ns, uint32_t amp, uint16_t offset) {

	for (int i = 0; i < ns; i++) {
        float sine = sin((i * (2*M_PI)/ns) + 1);
        float scaled = sine * ((ref_v + 1)/2);

        uint32_t s = (uint32_t)(scaled + ref_v/2);
		out[i] = s;
	}
}
...
void wavetable_create_sawtooth(
		uint16_t* out, uint16_t ref_v, uint16_t ns, uint32_t amp, uint16_t offset) {
	for (int i = 0; i < ns; i++) {
//		float f = (2/M_PI) * asin(sin(2*M_PI * i));
//		float saw = -((float)2/M_PI)
