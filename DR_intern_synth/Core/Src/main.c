...
float index_step = pow(OCTAVE_STEP, 12*4 + SEMITONE_A);
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    /* USER CODE BEGIN 3 */
    	...
    	if (i2s_tx_half) {
    		in_wave_index = wavetable_outwave_update(i2s_out, 0, I2S_OUT_N/2, out_wave_saw, in_wave_index, index_step);
    		i2s_tx_half = false;
    	}

		if (i2s_tx_cplt) {
			in_wave_index = wavetable_outwave_update(i2s_out, I2S_OUT_N/2, I2S_OUT_N, out_wave_saw, in_wave_index, index_step);
			i2s_tx_cplt = false;
		}
...
//				wavetable_create(SINE, i2s_out, REF_V_DIGITAL, ns_a4, 1);
				...
				index_step = pow(OCTAVE_STEP, 12*2 + SEMITONE_C);
				chord_progress = 0;
